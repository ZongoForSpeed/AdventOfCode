...
public final class Day04 {
    ...
    static int partOne(Scanner scanner) {
        CharMap map = CharMap.read(scanner, _ -> true);
        Set<Point2D> forklift = findForklift(map);
        return forklift.size();
    }

    static int partTwo(Scanner scanner) {
        CharMap map = CharMap.read(scanner, _ -> true);

        int rolls = 0;
        while (true) {
            Set<Point2D> toRemove = findForklift(map);
            if (toRemove.isEmpty()) {
                break;
            }
    ...
    private static Set<Point2D> findForklift(CharMap map) {
        Set<Point2D> forklift = new HashSet<>();
        Set<Point2D> points = new HashSet<>(map.points());
        for (Point2D point : points) {
            if (map.get(point) == '@') {
                long count = NEIGHBORS.stream().map(point::move)
