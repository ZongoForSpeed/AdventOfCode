...
public final class Day23 {
     ...
     * after cup 1?
     */
    static String crabCupsPart1(String input, int moves) {
        IntList cups = new IntArrayList();
        for (char c : input.toCharArray()) {
            cups.add(Character.getNumericValue(c));
        }
        CupCircle cupCircle = new CupCircle(cups.toIntArray());
        for (int move = 1; move <= moves; ++move) {
            cupCircle.performStep();
        }
        String result = cupCircle.resultPart1();
        LOGGER.info("Result crabCupsPart2({}, {}): {}", input, moves, result);
        return result;
    }
     ...
     * do you get if you multiply their labels together?
     s*/
    static long crabCupsPart2(String input, int moves, int size) {
        IntList cups = new IntArrayList();
        for (char c : input.toCharArray()) {
            cups.add(Character.getNumericValue(c));
        }
