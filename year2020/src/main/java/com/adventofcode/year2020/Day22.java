...
public final class Day22 {
    ...
    private static Pair<Integer, Long> playRecursiveCombat(Deque<Integer> player1deck, Deque<Integer> player2deck, int depth) {
        Set<Pair<IntList, IntList>> cache = new HashSet<>();
        int round = 0;
        while (!player1deck.isEmpty() && !player2deck.isEmpty()) {
            ...
            if (card1 <= player1deck.size() && card2 <= player2deck.size()) {
                Deque<Integer> newPlayer1Deck = player1deck.stream().limit(card1).collect(Collectors.toCollection(ArrayDeque::new));
                Deque<Integer> newPlayer2Deck = player2deck.stream().limit(card2).collect(Collectors.toCollection(ArrayDeque::new));
