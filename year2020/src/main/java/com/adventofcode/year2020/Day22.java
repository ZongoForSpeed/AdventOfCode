...
public final class Day22 {
    ...
    static long playCrabCombat(Scanner scanner) {
        var player1deck = new ArrayDeque<Integer>();
        var player2deck = new ArrayDeque<Integer>();
        readPlayerDecks(scanner, player1deck, player2deck);

        var round = 0;
        while (!player1deck.isEmpty() && !player2deck.isEmpty()) {
            var card1 = player1deck.removeFirst();
            var card2 = player2deck.removeFirst();
            if (card1 > card2) {
    ...
    /// player's score?
    static long playRecursiveCombat(Scanner scanner) {
        var player1deck = new ArrayDeque<Integer>();
        var player2deck = new ArrayDeque<Integer>();
        readPlayerDecks(scanner, player1deck, player2deck);

        var result = playRecursiveCombat(player1deck, player2deck, 1);
        return result.right();
    }

    private static void readPlayerDecks(Scanner scanner, Deque<Integer> player1deck, Deque<Integer> player2deck) {
        var readPlayer2 = false;
        while (scanner.hasNextLine()) {
            ...
            var card = Integer.parseInt(line);
            if (readPlayer2) {
    ...
    private static Pair<Integer, Long> playRecursiveCombat(Deque<Integer> player1deck, Deque<Integer> player2deck, int depth) {
        var cache = new HashSet<Pair<IntList, IntList>>();
        var round = 0;
        while (!player1deck.isEmpty() && !player2deck.isEmpty()) {
            if (!cache.add(Pair.of(new IntArrayList(player1deck), new IntArrayList(player2deck)))) {
            ...
            if (card1 <= player1deck.size() && card2 <= player2deck.size()) {
                var newPlayer1Deck = player1deck.stream().limit(card1).collect(Collectors.toCollection(ArrayDeque::new));
                var newPlayer2Deck = player2deck.stream().limit(card2).collect(Collectors.toCollection(ArrayDeque::new));

                LOGGER.debug("Launching sub-game with {} and {}", newPlayer1Deck, newPlayer2Deck);
                var subGameResult = playRecursiveCombat(newPlayer1Deck, newPlayer2Deck, depth + 1);
