...
public final class Day22 {
     ...
     */
    static long sporificaVirusPartOne(Scanner scanner, int steps) {
        BooleanMap booleanMap = BooleanMap.read(scanner, c -> c == '#');

        LOGGER.info("booleanMap:\n{}", booleanMap);
        int x = (booleanMap.xMax() + 1) / 2;
        int y = (booleanMap.yMax() + 1) / 2;

        InfiniteBooleanMap current = new InfiniteBooleanMap();
        ...
        for (int step = 1; step <= steps; ++step) {
            if (current.getOrDefault(position, false)) {
                current.remove(position);
                direction = direction.right();
            } else {
                bursts++;
                current.put(position, true);
     ...
     * cause a node to become infected? (Do not count nodes that begin infected.)
     */
    static long sporificaVirusPartTwo(Scanner scanner, int steps) {
        BooleanMap booleanMap = BooleanMap.read(scanner, c -> c == '#');
        ...
        InfiniteCharMap current = new InfiniteCharMap();
