...
public final class Day04 {
    ...
    public static long passportProcessing(List<String> batch) {
        ...
        return passports.stream()
                ...
                .filter(Day04::checkMandatoryFields)
                .count();
    }

    public static long runBatch(List<String> batch) {
        return readBatchFile(batch).stream()
                .map(Day04::readPassport)
                .filter(p -> checkMandatoryFields(p) && checkPassportFields(p))
                .count();
    }

    static Map<String, String> readPassport(List<String> passport) {
        return passport.stream().map(s -> s.split(":")).collect(Collectors.toMap(s -> s[0], s -> s[1]));
    }

    static boolean checkMandatoryFields(Map<String, String> fields) {
