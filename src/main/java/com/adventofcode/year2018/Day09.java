...
public final class Day09 {
     ...
     */
    static OptionalLong playMarblesMania(int players, int lastMarble) {
        MarblesCircle marblesCircle = new MarblesCircle();

        IntPriorityQueue marbles = new IntArrayFIFOQueue();
        IntStream.rangeClosed(1, lastMarble).forEach(marbles::enqueue);
        ...
        while (!marbles.isEmpty()) {
            int marble = marbles.dequeueInt();
            if (marble % 23 == 0) {
                marblesCircle.moveAntiClockWise(7);
                scores[player - 1] += marble + marblesCircle.remove();
            } else {
                marblesCircle.moveClockWise(2);
                marblesCircle.add(marble);
            }

            LOGGER.trace("[{}] {}", player, marblesCircle);
    ...
    static class MarblesCircle {
        private final IntArrayFIFOQueue deque;

        MarblesCircle() {
            deque = new IntArrayFIFOQueue();
            deque.enqueue(0);
        }

        void moveClockWise() {
            int first = deque.dequeueInt();
            deque.enqueue(first);
        }

        void moveClockWise(int move) {
        ...
        void moveAntiClockWise() {
            int last = deque.dequeueLastInt();
            deque.enqueueFirst(last);
        }

        void moveAntiClockWise(int move) {
            for (int i = 1; i <= move; ++i) {
                moveAntiClockWise();
            }
        }
        ...
        void add(int marble) {
            deque.enqueueFirst(marble);
        }

        int remove() {
            return deque.dequeueInt();
        }
