...
class Day17Test {
    ...
    private static long findCombinationsPartOne(Scanner scanner, int size) {
        Long2LongMap combinations = findCombinations(scanner, size);

        return combinations.values().longStream().sum();
    }

    private static long findCombinationsPartTwo(Scanner scanner, int size) {
        Long2LongMap combinations = findCombinations(scanner, size);
    ...
    private static Long2LongMap findCombinations(Scanner scanner, int size) {
        ...
        for (long c = 1; c < maxBitSet; ++c) {
            ...
            if (sum == size) {
                combinations.compute(set.cardinality(), (ignore, value) -> value == null ? 1 : value + 1);
            }
        }
    ...
    void inputExample() {
        ...
        assertThat(findCombinationsPartOne(new Scanner(input), 25)).isEqualTo(4);
        assertThat(findCombinationsPartTwo(new Scanner(input), 25)).isEqualTo(3);
    }
    ...
    void inputPartOne() throws IOException {
        try (InputStream is = Day17Test.class.getResourceAsStream("/2015/day/17/input")) {
            Scanner scanner = new Scanner(Objects.requireNonNull(is));
            assertThat(findCombinationsPartOne(scanner, 150)).isEqualTo(4372);
        }
    }
    ...
    void inputPartTwo() throws IOException {
        try (InputStream is = Day17Test.class.getResourceAsStream("/2015/day/17/input")) {
            Scanner scanner = new Scanner(Objects.requireNonNull(is));
            assertThat(findCombinationsPartTwo(scanner, 150)).isEqualTo(4);
        }
    }
}
