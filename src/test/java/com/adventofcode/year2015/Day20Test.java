...
class Day20Test {
    ...
    Day20Test() {
        primes = Prime.sieve(1_000_000);
    }

    private long countPresent(long houseNumber) {
        return Arithmetic.sigma(houseNumber, primes) * 10;
    }

    private long findLowestHouseNumberPartOne(long gifts) {
        for (int h = 1; ; ++h) {
            if (countPresent(h) > gifts) {
                return h;
            }
        }
    }

    private long findLowestHouseNumberPartTwo(long gifts) {
        int limit = 1_000_000;
        int[] memory = new int[limit];
        for (int elf = 1; elf < limit; ++elf) {
            for (int house = 1; house <= 50; ++house) {
                if (house * elf >= limit) {
                    break;
                }
                memory[house * elf] += 11 * elf;
            }
        }

        for (int house = 1; ; ++house) {
            if (memory[house] > gifts) {
                return house;
            }
        }
    }
    ...
    @Test
    void inputExample() {
        assertThat(countPresent(1)).isEqualTo(10);
        assertThat(countPresent(4)).isEqualTo(70);
        assertThat(countPresent(6)).isEqualTo(120);
    }
    ...
    void inputPartOne() throws IOException {
        assertThat(findLowestHouseNumberPartOne(29000000)).isEqualTo(665280);
    }
    ...
    void inputPartTwo() throws IOException {
        assertThat(findLowestHouseNumberPartTwo(29000000)).isEqualTo(705600);
    }
}
