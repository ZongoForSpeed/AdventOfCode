...
class Day22Test {
    ...
    private static List<AStar.Move<State>> nextStatePartOne(State state) {
        return nextState(state, false);
    }

    private static List<AStar.Move<State>> nextStatePartTwo(State state) {
        return nextState(state, true);
    }

    private static List<AStar.Move<State>> nextState(State state, boolean hardMode) {
    ...
    private static Boss readBossStats(Scanner scanner) {
    ...
    private static long gamePartOne(Player player, Boss boss) {
        return AStar.algorithm(Day22Test::nextStatePartOne, PlayableState.of(player, boss, true), WINNING_STATE);
    }

    private static long gamePartTwo(Player player, Boss boss) {
        return AStar.algorithm(Day22Test::nextStatePartTwo, PlayableState.of(player, boss, true), WINNING_STATE);
    }

    @Test
    void inputSmallExample() {
        Player player = Player.of(10, 250, new EnumMap<>(Spells.class));
        Boss boss = Boss.of(13, 8);

        assertThat(gamePartOne(player, boss)).isEqualTo(226);
    }

    @Test
    void inputLargeExample() {
        Player player = Player.of(10, 250, new EnumMap<>(Spells.class));
        Boss boss = Boss.of(14, 8);

        assertThat(gamePartOne(player, boss)).isEqualTo(641);
    }
    ...
    void inputPartOne() throws IOException {
        try (InputStream is = Day22Test.class.getResourceAsStream("/2015/day/22/input")) {
            Scanner scanner = new Scanner(Objects.requireNonNull(is));

            Boss boss = readBossStats(scanner);
            Player player = Player.of(50, 500, new EnumMap<>(Spells.class));

            assertThat(gamePartOne(player, boss)).isEqualTo(953);
        }
    }
    ...
    void inputPartTwo() throws IOException {
        try (InputStream is = Day22Test.class.getResourceAsStream("/2015/day/22/input")) {
            ...
            long mana = gamePartTwo(player, boss);
            assertThat(mana).isEqualTo(1289);
        }
    }
    ...
    record PlayableState(Player player, Boss boss, boolean turn) implements State {
        public static PlayableState of(Player player, Boss boss, boolean turn) {
            return new PlayableState(player, boss, turn);
        }
    }

    record TerminalState(boolean winner) implements State {
        public static TerminalState of(boolean winner) {
            return new TerminalState(winner);
        }
    }

    record Player(int hitPoints, int mana, Map<Spells, Integer> effects) {
