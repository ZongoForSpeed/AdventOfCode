...
class Day22Test {
    ...
    private static List<GridNode> parseGridNodes(Scanner scanner) {
    ...
    private static long distance(State a, State b) {
        return distance(a.empty(), a.goal()) + distance(a.goal(), b.goal()) + distance(b.empty(), b.goal());
    }

    private static long countSteps(Scanner scanner, Point2D goal) {
        ...
        return AStar.algorithmHeuristic(state -> state.nextStates(graph), (a, b) -> 1L, Day22Test::distance, State.of(goal, empty), State.of(Point2D.of(0, 0), Point2D.of(1, 0)));
    }
    ...
    void inputExample() {
        ...
        long steps = countSteps(scanner, Point2D.of(2, 0));
    ...
    void inputPartOne() throws IOException {
        try (InputStream is = Day22Test.class.getResourceAsStream("/2016/day/22/input")) {
            ...
            assertThat(viablePairs).isEqualTo(1034);
        }
    }
    ...
    void inputPartTwo() throws IOException {
        try (InputStream is = Day22Test.class.getResourceAsStream("/2016/day/22/input")) {
            Scanner scanner = new Scanner(Objects.requireNonNull(is));
            assertThat(countSteps(scanner, Point2D.of(37, 0))).isEqualTo(261);
        }
    }
