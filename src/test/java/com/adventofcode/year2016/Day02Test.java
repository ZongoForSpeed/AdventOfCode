...
class Day02Test {
    ...
    private static String findCodePartOne(Scanner scanner) {
        CharMap grid = CharMap.read("""
                123
                456
                789""", c -> c != ' ');

        return findCode(scanner, grid);
    }

    private static String findCode(Scanner scanner, CharMap grid) {
        Set<Point2D> availablePosition = new HashSet<>(grid.points());

        StringBuilder sb = new StringBuilder();

        Point2D position = availablePosition.stream().filter(p -> grid.get(p) == '5').findFirst().orElseThrow();
        while (scanner.hasNextLine()) {
            String line = scanner.nextLine();
            for (char c : line.toCharArray()) {
                Point2D nextPosition = switch (c) {
                ...
                if (availablePosition.contains(nextPosition)) {
                    position = nextPosition;
                }
                LOGGER.trace("Current position after '{}': {}", c, position);
            }
            sb.append(grid.get(position));
        }
        return sb.toString();
    }

    private static String findCodePartTwo(Scanner scanner) {
        CharMap grid = CharMap.read("""
                  1
                 234
                56789
    ...
    void inputExample() {
        ...
        assertThat(findCodePartOne(new Scanner(input))).isEqualTo("1985");
        assertThat(findCodePartTwo(new Scanner(input))).isEqualTo("5DB3");
    }
    ...
    void inputPartOne() throws IOException {
        try (InputStream is = Day02Test.class.getResourceAsStream("/2016/day/2/input")) {
            Scanner scanner = new Scanner(Objects.requireNonNull(is));
            assertThat(findCodePartOne(scanner)).isEqualTo("97289");
        }
    }
    ...
    void inputPartTwo() throws IOException {
        try (InputStream is = Day02Test.class.getResourceAsStream("/2016/day/2/input")) {
            Scanner scanner = new Scanner(Objects.requireNonNull(is));
            assertThat(findCodePartTwo(scanner)).isEqualTo("9A7DC");
        }
    }
