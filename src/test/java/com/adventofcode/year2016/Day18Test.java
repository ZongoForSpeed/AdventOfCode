...
class Day18Test {
    ...
    private static BooleanMap getTiles(String input, int rows) {
    ...
    void inputExample() {
        String input = "..^^.";
        BooleanMap booleanMap = getTiles(input, 3);
    ...
    void inputLargerExample() {
        String input = ".^^.^.^^^^";
        BooleanMap booleanMap = getTiles(input, 10);
        assertThat(booleanMap).hasToString("""
                .##.#.####
                ###...#..#
                #.##.#.##.
    ...
    void inputPartOne() throws IOException {
        try (InputStream is = Day18Test.class.getResourceAsStream("/2016/day/18/input")) {
            Scanner scanner = new Scanner(Objects.requireNonNull(is));
            BooleanMap booleanMap = getTiles(scanner.nextLine(), 40);
            int size = booleanMap.yMax() * booleanMap.xMax();
            assertThat(size - booleanMap.cardinality()).isEqualTo(1987);
        }
    }
    ...
    void inputPartTwo() throws IOException {
        try (InputStream is = Day18Test.class.getResourceAsStream("/2016/day/18/input")) {
            Scanner scanner = new Scanner(Objects.requireNonNull(is));
            BooleanMap booleanMap = getTiles(scanner.nextLine(), 400000);
            long size = (long) booleanMap.yMax() * booleanMap.xMax();
            assertThat(size - booleanMap.cardinality()).isEqualTo(19984714);
        }
    }
}
