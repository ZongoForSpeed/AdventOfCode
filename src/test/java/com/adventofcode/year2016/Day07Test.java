...
class Day07Test {
    ...
    private static boolean abba(String input) {
    ...
    private static List<String> findABA(String input) {
    ...
    private static String correspondingBAB(String aba) {
        if (aba.length() != 3) {
    ...
    private static boolean supportTLS(String ipV7) {
        ...
        return outsideBrackets.stream().anyMatch(Day07Test::abba)
                && insideBrackets.stream().noneMatch(Day07Test::abba);
    }

    private static boolean supportSSL(String ipV7) {
        List<String> outsideBrackets = new ArrayList<>();
        List<String> insideBrackets = new ArrayList<>();
        ...
        List<String> bab = outsideBrackets.stream()
                .map(Day07Test::findABA)
                .flatMap(Collection::stream)
                .map(Day07Test::correspondingBAB)
                .toList();
    ...
    private static int supportTLS(Scanner scanner) {
        int count = 0;
    ...
    private static int supportSSL(Scanner scanner) {
        int count = 0;
        while (scanner.hasNextLine()) {
            String line = scanner.nextLine();
            if (supportSSL(line)) {
    ...
    @Test
    void inputExample() {
        assertThat(abba("abba")).isTrue();
        assertThat(abba("aaaa")).isFalse();
        assertThat(abba("xyyx")).isTrue();

        assertThat(supportTLS("abba[mnop]qrst")).isTrue();
        assertThat(supportTLS("abcd[bddb]xyyx")).isFalse();
        assertThat(supportTLS("aaaa[qwer]tyui")).isFalse();
        assertThat(supportTLS("ioxxoj[asdfgh]zxcvbn")).isTrue();

        assertThat(supportSSL("aba[bab]xyz")).isTrue();
        assertThat(supportSSL("xyx[xyx]xyx")).isFalse();
        assertThat(supportSSL("aaa[kek]eke")).isTrue();
        assertThat(supportSSL("zazbz[bzb]cdb")).isTrue();
    }
    ...
    void inputPartOne() throws IOException {
        try (InputStream is = Day07Test.class.getResourceAsStream("/2016/day/7/input")) {
            Scanner scanner = new Scanner(Objects.requireNonNull(is));
            assertThat(supportTLS(scanner)).isEqualTo(118);
        }
    }
    ...
    void inputPartTwo() throws IOException {
        try (InputStream is = Day07Test.class.getResourceAsStream("/2016/day/7/input")) {
            Scanner scanner = new Scanner(Objects.requireNonNull(is));
            assertThat(supportSSL(scanner)).isEqualTo(260);
        }
    }
}
