...
public class Day08Test {
    ...
    Day08Test() {
        mappings = Permutations.permutations("abcdefg").stream().map(Day08Test::getMapping).toList();
    }

    private static long count1478(Scanner scanner) {
        long count = 0;
    ...
    private int solveMapping(String line) {
        ...
        List<String> left = Arrays.stream(split[0].split(" ")).map(Day08Test::sort).toList();
        List<String> right = Arrays.stream(split[1].split(" ")).map(Day08Test::sort).toList();
    ...
    private int solveMapping(Scanner scanner) {
        int sum = 0;
        while (scanner.hasNextLine()) {
    ...
    void inputExample() {
        ...
        assertThat(count1478(new Scanner(input))).isEqualTo(26);

        assertThat(solveMapping("acedgfb cdfbe gcdfa fbcad dab cefabd cdfgeb eafb cagedb ab | cdfeb fcadb cdfeb cdbaf")).isEqualTo(5353);

        Scanner scanner = new Scanner(input);

        assertThat(solveMapping(scanner)).isEqualTo(61229);
    }
