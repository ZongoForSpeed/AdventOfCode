...
class Day11Test {
    ...
    private static int dumboOctopusPartOne(Scanner scanner, int steps) {
        ...
        Map<Point2D, List<Point2D>> adjacents = points.stream().collect(Collectors.toMap(p -> p, p -> adjacent(p, xMax, yMax)));

        int countFlashes = 0;
        for (int step = 1; step <= steps; ++step) {
            Set<Point2D> flashes = nextStep(points, map, adjacents);
            LOGGER.info("After step {}:\n{}", step, map);
            countFlashes += flashes.size();
        }
    ...
    private static int dumboOctopusPartTwo(Scanner scanner) {
        ...
        for (int step = 1; ; ++step) {
            Set<Point2D> flashes = nextStep(points, map, adjacents);
            LOGGER.info("After step {}:\n{}", step, map);
            if (flashes.size() == points.size()) {
                return step;
            }
        }
    }

    private static Set<Point2D> nextStep(List<Point2D> points, IntegerMap map, Map<Point2D, List<Point2D>> adjacents) {
        Set<Point2D> flashes = new HashSet<>();
        for (Point2D point : points) {
            if (map.increment(point, 1) > 9) {
                flashes.add(point);
            }
        }
    ...
    void inputSimpleExample() {
        ...
        assertThat(dumboOctopusPartOne(new Scanner(input), 2)).isEqualTo(9);
    ...
    void inputExample() {
        ...
        assertThat(dumboOctopusPartOne(new Scanner(input), 10)).isEqualTo(204);
        assertThat(dumboOctopusPartOne(new Scanner(input), 100)).isEqualTo(1656);
        assertThat(dumboOctopusPartTwo(new Scanner(input))).isEqualTo(195);
    }
