...
class Day22Test {
    ...
    private static final Cuboid LIMIT = Cuboid.of(-50, 50, -50, 50, -50, 50);
     ...
     */
    private static long computeSize(Scanner scanner, Cuboid limit) {
        List<Pair<Boolean, Cuboid>> cuboids = new ArrayList<>();

        while (scanner.hasNextLine()) {
            String line = scanner.nextLine();
            Matcher matcher = CUBES_PATTERN.matcher(line);
            if (matcher.matches()) {
                String on_off = matcher.group(1);
                String dx = matcher.group(2);
                String dy = matcher.group(3);
                String dz = matcher.group(4);

                Cuboid cuboid = Cuboid.of(Range.of(dx), Range.of(dy), Range.of(dz));
                if (limit == null || limit.contains(cuboid)) {
                    LOGGER.debug("{} / cuboid={}", on_off, cuboid);
                    cuboids.add(Pair.of("on".equals(on_off), cuboid));
                }
            }
        }
        ...
        for (Pair<Boolean, Cuboid> pair : cuboids) {
            ...
            LOGGER.info("Sets after treating {}: {}", pair, sets.size());
        }

        LOGGER.trace("Sets : {}", sets);
        long size = sets.stream().mapToLong(Cuboid::size).sum();
    ...
    void largerExample() {
        ...
        assertThat(computeSize(new Scanner(input), LIMIT)).isEqualTo(474140);
        assertThat(computeSize(new Scanner(input), null)).isEqualTo(2758514936282235L);
    }
    ...
    void simpleExample() {
        ...
        assertThat(computeSize(new Scanner(input), LIMIT)).isEqualTo(39);
    }
    ...
    void inputExample() {
        ...
        assertThat(computeSize(new Scanner(input), LIMIT)).isEqualTo(590784L);
    }

    @Test
    void inputPartOne() throws IOException {
        try (InputStream is = Day22Test.class.getResourceAsStream("/2021/day/22/input")) {
            Scanner scanner = new Scanner(Objects.requireNonNull(is));
            assertThat(computeSize(scanner, LIMIT)).isEqualTo(652209L);
        }
    }
    ...
    void inputPartTwo() throws IOException {
        try (InputStream is = Day22Test.class.getResourceAsStream("/2021/day/22/input")) {
            Scanner scanner = new Scanner(Objects.requireNonNull(is));
            assertThat(computeSize(scanner, null)).isEqualTo(1217808640648260L);
        }
    }

    record Cuboid(Range dx, Range dy, Range dz) {
