...
public class Day05Test {
    ...
    void inputExample() {
        ...
        assertThat(countOverlapsPartOne(new Scanner(input))).isEqualTo(5);
        assertThat(countOverlapsPartTwo(new Scanner(input))).isEqualTo(12);

    }

    private int countOverlapsPartOne(Scanner scanner) {
    ...
    private int countOverlapsPartTwo(Scanner scanner) {
        IntegerMap map = new IntegerMap(10, 10, 0);
        Pattern pattern = Pattern.compile("(\\d+),(\\d+) -> (\\d+),(\\d+)");
        while (scanner.hasNextLine()) {
            String line = scanner.nextLine();
