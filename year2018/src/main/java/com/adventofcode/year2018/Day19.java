...
public final class Day19 {
    ...
    static int executePartTwo(Scanner scanner) {
        ...
        int objective = Integer.MIN_VALUE;
        ...
        while (true) {
            ...
            LOGGER.trace("Register after: {}", register);
            register.set(ip, register.getInt(ip) + 1);
            if (objective < register.getInt(4)) {
                objective = register.getInt(4);
                if (register.getInt(0) == 0 && register.getInt(1) == register.getInt(2)) {
                    LOGGER.debug("Max found = {}", register);
                    break;
                }
            }
        }
