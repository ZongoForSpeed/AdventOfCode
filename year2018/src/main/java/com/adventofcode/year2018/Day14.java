...
public final class Day14 {
     ...
     */
    static IntList chocolateChartsPartOne(int shift) {
        IntList recipes = new IntArrayList();
        recipes.add((char) 3);
        recipes.add((char) 7);
        int firstElf = 0;
        int secondElf = 1;

        while (recipes.size() < 10 + shift) {
            int firstRecipe = recipes.getInt(firstElf);
            int secondRecipe = recipes.getInt(secondElf);

            CharList digits = Digits.digits(firstRecipe + secondRecipe);
            digits.forEach(recipes::add);
            firstElf += firstRecipe + 1;
            firstElf %= recipes.size();
            secondElf += secondRecipe + 1;
            secondElf %= recipes.size();

            LOGGER.trace("recipes = {}", recipes);
        }

        IntList lastRecipes = recipes.subList(shift, 10 + shift);
        LOGGER.info("lastRecipes = {}", lastRecipes);
        return lastRecipes;
    }
     ...
     */
    static int chocolateChartsPartTwo(int pattern) {
        CharList patternList = Digits.digits(pattern);
        CharList recipes = new CharArrayList();
        recipes.add((char) 3);
        recipes.add((char) 7);
        int firstElf = 0;
        int secondElf = 1;
        ...
        while (true) {
            int firstRecipe = recipes.getChar(firstElf);
            int secondRecipe = recipes.getChar(secondElf);

            CharList digits = Digits.digits(firstRecipe + secondRecipe);
            recipes.addAll(digits);
            OptionalInt checkPattern = checkPattern(recipes, patternList);
            if (checkPattern.isPresent()) {
                return checkPattern.getAsInt();
            }
            firstElf += firstRecipe + 1;
            firstElf %= recipes.size();
            secondElf += secondRecipe + 1;
    ...
    static OptionalInt checkPattern(CharList recipes, CharList pattern) {
        if (pattern.size() >= recipes.size()) {
            return OptionalInt.empty();
        }
        CharList integers = recipes.subList(recipes.size() - pattern.size(), recipes.size());
        if (ListUtils.isEqualList(integers, pattern)) {
            return OptionalInt.of(recipes.size() - pattern.size());
        }

        if (recipes.size() > pattern.size()) {
            integers = recipes.subList(recipes.size() - pattern.size() - 1, recipes.size() - 1);
            if (ListUtils.isEqualList(integers, pattern)) {
                return OptionalInt.of(recipes.size() - pattern.size() - 1);
            }
        }
