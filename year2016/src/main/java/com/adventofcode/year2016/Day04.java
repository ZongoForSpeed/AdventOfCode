...
public final class Day04 {
    ...
    private static @Nullable Pair<String, Integer> checkRoomName(String name) {
    ...
    private static @Nullable Pair<String, Integer> decryptRoomName(String input) {
        var pair = checkRoomName(input);
        if (pair == null) {
            return null;
        }

        String name = pair.left();
        var sectorID = pair.right();
    ...
    static int checkRoomName(Scanner scanner) {
        ...
        while (scanner.hasNextLine()) {
            var pair = checkRoomName(scanner.nextLine());
            if (pair != null) {
    ...
    static int decryptRoomName(Scanner scanner) {
        while (scanner.hasNextLine()) {
            var pair = decryptRoomName(scanner.nextLine());
