...
class Day01Test {
     ...
     * Elf carrying?
     */
    public static int maxCalorieCounting(Scanner scanner) {
        return readInput(scanner).stream().map(IntCollection::intStream).mapToInt(IntStream::sum).max().orElseThrow();
    }

    private static List<IntList> readInput(Scanner scanner) {
        List<IntList> elves = new ArrayList<>();
        IntList currentElf = new IntArrayList();
        while (scanner.hasNextLine()) {
            String line = scanner.nextLine();
            if (StringUtils.isBlank(line)) {
                elves.add(currentElf);
                currentElf = new IntArrayList();
            } else {
    ...
    void testSampleInput() {
        ...
        assertThat(maxCalorieCounting(new Scanner(input))).isEqualTo(24000);

        assertThat(topCalorieCounting(new Scanner(input))).isEqualTo(45000);
    ...
    void inputPartOne() throws IOException {
        try (InputStream is = Day01Test.class.getResourceAsStream("/2022/day/01/input");
            ...
            assertThat(maxCalorieCounting(scanner)).isEqualTo(69693);
        }
    }
     ...
     */
    public static int topCalorieCounting(Scanner scanner) {
        return readInput(scanner).stream().map(IntCollection::intStream).map(IntStream::sum).sorted(Comparator.reverseOrder()).limit(3).mapToInt(i -> i).sum();
    }
    ...
    void inputPartTwo() throws IOException {
        try (InputStream is = Day01Test.class.getResourceAsStream("/2022/day/01/input");
             Scanner scanner = new Scanner(Objects.requireNonNull(is))) {
            assertThat(topCalorieCounting(scanner)).isEqualTo(200945);
        }
    }
