...
public final class Day11 {
    ...
    private static final Pattern ITEM_PATTERN = Pattern.compile("  Starting items: (.*)");
    private static final Pattern OPERATION_PATTERN = Pattern.compile("  Operation: new = (\\S+) (\\S) (\\S+)");
    ...
    public static Long partOne(Scanner scanner) {
        List<Monkey> monkeys = readMonkeys(scanner);
        ...
        for (int round = 1; round <= 20; ++round) {
            ...
            for (Monkey monkey : monkeys) {
                while (!monkey.items.isEmpty()) {
                    ...
                    monkey.inspected++;
                    item = monkey.operation.applyAsLong(item) / 3;

                    int toMonkey = monkey.test.test(item) ? monkey.monkeyTrue : monkey.monkeyFalse;
                    monkeys.get(toMonkey).items.addLast(item);
                }
            }

            for (Monkey monkey : monkeys) {
                LOGGER.info("Monkey {}: {}", monkey.id, monkey.items);
            }
        }

        return monkeys.stream().map(m -> m.inspected)
    ...
    public static Long partTwo(Scanner scanner) {
        ...
        long globalModulo = monkeys.stream().mapToLong(m -> m.divisor).reduce(1L, (a, b) -> a * b);
        LOGGER.info("Monkeys: {}", monkeys);
        ...
        for (int round = 1; round <= 10000; ++round) {
            for (Monkey monkey : monkeys) {
                while (!monkey.items.isEmpty()) {
                    Long item = monkey.items.pollFirst();
                    monkey.inspected++;
                    item = monkey.operation.applyAsLong(item) % globalModulo;

                    int toMonkey = monkey.test.test(item) ? monkey.monkeyTrue : monkey.monkeyFalse;
                    Objects.requireNonNull(monkeys.get(toMonkey)).items.addLast(item);
                }
            }
    ...
    private static List<Monkey> readMonkeys(Scanner scanner) {
        ...
        while (scanner.hasNextLine()) {
            ...
            if (matcher.matches()) {
                ...
                LOGGER.info("Operation = '{}', '{}', '{}'", left, op, right);
                LongUnaryOperator operation = readOperation(left, op, right);
                LOGGER.info("Operation = '{}'", operation);
                monkey.operation = operation;
                continue;
            }
            matcher = TEST_PATTERN.matcher(line);
            if (matcher.matches()) {
                long test = Long.parseLong(matcher.group(1));
                LOGGER.info("Test = {}", test);
                monkey.divisor = test;
                monkey.test = t -> t % test == 0;
                continue;
            }
            matcher = IF_PATTERN.matcher(line);
            if (matcher.matches()) {
                String test = matcher.group(1);
                int monkeyTo = Integer.parseInt(matcher.group(2));
                LOGGER.info("if '{}' => '{}", test, monkey);
                if ("true".equals(test)) {
                    monkey.monkeyTrue = monkeyTo;
                } else {
                    monkey.monkeyFalse = monkeyTo;
                }
                continue;
            }
    ...
    private static LongUnaryOperator readOperation(String left, String op, String right) {
        Operation operation = OPERATIONS.get(op);
        if (operation == null) {
            throw new IllegalStateException("Cannot find operation : " + op);
        }

        if ("old".equals(left) && "old".equals(right)) {
        ...
        if ("old".equals(left)) {
            long parsed = Long.parseLong(right);
            return new LongUnaryOperator() {
                @Override
                public long applyAsLong(long operand) {
                    return operation.operation(operand, parsed);
                }
        ...
        if ("old".equals(right)) {
            long parsed = Long.parseLong(left);
            return new LongUnaryOperator() {
                @Override
                public long applyAsLong(long operand) {
                    return operation.operation(parsed, operand);
                }
        ...
        throw new IllegalStateException("Cannot readOperation(" + left + ", " + op + ", " + right + ")");
    }
