...
public final class Day03 {
    ...
    static final class PartOne {
        ...
        static int sumPriorities(Scanner scanner) {
            int sum = 0;
            while (scanner.hasNextLine()) {
                String rucksack = scanner.nextLine();
                sum += PartOne.rucksackPriority(rucksack);
            }
            return sum;
        }

        static int rucksackPriority(String rucksack) {
            int position = rucksack.length() / 2;
            String first = rucksack.substring(0, position);
            String second = rucksack.substring(position);


            Set<Integer> firstSet = first.chars().boxed().collect(Collectors.toSet());
            Set<Integer> secondSet = second.chars().boxed().collect(Collectors.toSet());

            SetUtils.SetView<Integer> intersection = SetUtils.intersection(firstSet, secondSet);

            LOGGER.info("first = '{}', second = '{}' ==> {}", first, second, intersection);

            return intersection.stream().mapToInt(PartOne::priority).findFirst().orElseThrow();
        }
        ...
        static int priority(int c) {
            ...
            throw new IllegalStateException("Unknown code point: " + c);
        }
    }
    ...
    static final class PartTwo {
        ...
        static int rucksackPriority(String first, String second, String third) {

            Set<Integer> firstSet = first.chars().boxed().collect(Collectors.toSet());
            Set<Integer> secondSet = second.chars().boxed().collect(Collectors.toSet());
            Set<Integer> thirdSet = third.chars().boxed().collect(Collectors.toSet());

            SetUtils.SetView<Integer> intersection = SetUtils.intersection(SetUtils.intersection(firstSet, secondSet), thirdSet);
        ...
        static int sumPriorities(Scanner scanner) {
            ...
            while (scanner.hasNextLine()) {
                ...
                sum += rucksackPriority(first, second, third);
            }
            return sum;
        }
    }
