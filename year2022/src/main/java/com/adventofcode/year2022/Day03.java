...
public final class Day03 {
    ...
    static final class PartOne {
        ...
        static int rucksackPriority(String rucksack) {
            var position = rucksack.length() / 2;
            String first = rucksack.substring(0, position);
            String second = rucksack.substring(position);


            var firstSet = first.chars().boxed().collect(Collectors.toSet());
            var secondSet = second.chars().boxed().collect(Collectors.toSet());

            var intersection = SetUtils.intersection(firstSet, secondSet);
    ...
    static final class PartTwo {
        ...
        static int rucksackPriority(String first, String second, String third) {
            ...
            var firstSet = first.chars().boxed().collect(Collectors.toSet());
            var secondSet = second.chars().boxed().collect(Collectors.toSet());
            var thirdSet = third.chars().boxed().collect(Collectors.toSet());

            var intersection = SetUtils.intersection(SetUtils.intersection(firstSet, secondSet), thirdSet);
