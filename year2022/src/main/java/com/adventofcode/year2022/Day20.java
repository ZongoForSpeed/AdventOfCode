...
public final class Day20 {
     ...
     */
    public static long decryptPartOne(Scanner scanner) {
        return decrypt(scanner, 1, 1);
    }
     ...
     */
    public static long decryptPartTwo(Scanner scanner) {
        return decrypt(scanner, DECRYPTION_KEY, 10);
    }

    private static long decrypt(Scanner scanner, int key, int mixing) {
        List<IntLongPair> file = readInitialList(scanner, key);
        List<IntLongPair> indices = List.copyOf(file);

        int reduceLength = file.size() - 1;
        for (int mix = 0; mix < mixing; ++mix) {
            for (IntLongPair index : indices) {
                long copyIndex = index.secondLong();
                if (copyIndex == 0) {
                    continue;
                }
                int position = file.indexOf(index);
                int newPosition = (int) ((position + copyIndex) % reduceLength);
                if (newPosition < 0) {
                    newPosition += reduceLength;
                }
                file.remove(position);
                file.add(newPosition, index);
                LOGGER.trace("After moving {}", index);
        ...
        int indexOfZero = IntStream.range(0, file.size()).filter(i -> file.get(i).secondLong() == 0).findFirst().orElse(0);

        long decrypted = 0;
        long decrypted1000 = file.get((indexOfZero + 1000) % file.size()).secondLong();
        LOGGER.info("1000th = {}", decrypted1000);
        decrypted += decrypted1000;
        long decrypted2000 = file.get((indexOfZero + 2000) % file.size()).secondLong();
        LOGGER.info("2000th = {}", decrypted2000);
        decrypted += decrypted2000;
        long decrypted3000 = file.get((indexOfZero + 3000) % file.size()).secondLong();
        LOGGER.info("3000th = {}", decrypted3000);
        decrypted += decrypted3000;
        return decrypted;
    }
    ...
    private static List<IntLongPair> readInitialList(Scanner scanner, int key) {
        ...
        while (scanner.hasNextLine()) {
            file.add(IntLongPair.of(position++, key * Long.parseLong(scanner.nextLine())));
        }
