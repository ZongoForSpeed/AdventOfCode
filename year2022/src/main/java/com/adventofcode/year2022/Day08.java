...
public final class Day08 {
    ...
    static final class PartOne {
        ...
        static long countVisibleTrees(Scanner scanner) {
            IntegerMap trees = IntegerMap.read(scanner);
            ...
            BooleanMap visible = new BooleanMap(xMax, yMax);

            for (int y = 0; y <= yMax; ++y) {
                int finalY = y;
                int[] line = IntStream.rangeClosed(0, xMax).map(x -> trees.get(x, finalY)).toArray();
                visible(line).forEach(x -> visible.set(x, finalY));
            }

            for (int x = 0; x <= xMax; ++x) {
                int finalX = x;
                int[] line = IntStream.rangeClosed(0, yMax).map(y -> trees.get(finalX, y)).toArray();
                visible(line).forEach(y -> visible.set(finalX, y));
            }
        ...
        static IntSet visible(int[] trees) {
            IntSet visible = new IntArraySet();
    ...
    static final class PartTwo {
        ...
        static int maxScenicScore(Scanner scanner) {
            IntegerMap trees = IntegerMap.read(scanner);
            ...
            int[][] rows = new int[xMax + 1][yMax + 1];
            int[][] columns = new int[xMax + 1][yMax + 1];

            for (int x = 0; x <= xMax; x++) {
                for (int y = 0; y <= yMax; y++) {
                    columns[x][y] = trees.get(x, y);
                    rows[y][x] = trees.get(x, y);
                }
            }
            ...
            for (int x = 0; x <= xMax; x++) {
                for (int y = 0; y <= yMax; y++) {
                    ...
                    IntIntPair scoreX = scenicScore(y, column);
                    IntIntPair scoreY = scenicScore(x, row);

                    int scenicScore = scoreX.firstInt() * scoreX.secondInt() * scoreY.firstInt() * scoreY.secondInt();
        ...
        static IntIntPair scenicScore(int position, int[] trees) {
            int currentTree = trees[position];
            int visible1 = 0;
            for (int i = position + 1; i < trees.length; i++) {
                visible1++;
            ...
            return IntIntPair.of(visible1, visible2);
        }
