...
public final class Day25 {
    ...
    /// Only 49 stars to go.
    /// ```
    public static Result partOne(Scanner scanner) {
        var sum = 0L;
        while (scanner.hasNextLine()) {
            String line = scanner.nextLine();
            var decimal = snafuToDecimal(line);
            LOGGER.info("{} -> {}", line, decimal);
            sum += decimal;
        }

        String snafu = decimalToSNAFU(sum);
        return new Result(sum, snafu);
    }

    public static long snafuToDecimal(String n) {
        var result = 0L;
        for (var i = 0; i < n.length(); ++i) {
            switch (n.charAt(i)) {
                case '0' -> result *= 5;
                case '1' -> result = 5 * result + 1;
                case '2' -> result = 5 * result + 2;
                case '-' -> result = 5 * result - 1;
                case '=' -> result = 5 * result - 2;
                default -> throw new IllegalStateException("Unknown character at " + i + " in '" + n + "'");
            }
        }
        return result;
    }
    ...
    public record Result(long decimal, String snafu) {
    }
}
