...
public final class Day10 {
    ...
    static final class PartOne {
        ...
        static long getSignalStrength(Scanner scanner) {
            IntList register = getRegister(scanner);

            var signalStrength = 0L;
            for (var cycle = 20; cycle <= register.size(); cycle += 40) {
    ...
    static final class PartTwo {
        ...
        static String printCRT(Scanner scanner) {
            IntList register = getRegister(scanner);

            var crt = new StringBuilder();
            var line = 1;
            var position = 0;
            for (var cycle = 1; cycle < register.size(); ++cycle) {
                crt.append(getSprite(position, register.getInt(cycle)));
        ...
        static char getSprite(int position, int x) {
            if (x <= position + 1 && position < x + 2) {
                return '#';
            } else {
                return '.';
            }
            // String rightPart = "###" + ".".repeat(80);
            // String sprite;
            // if (x < 1) {
            //     sprite = rightPart.substring(1 - x);
            // } else {
            //     sprite = ".".repeat(x - 1) + rightPart;
            // }
            // return sprite.charAt(position);
        }
    }

    private static IntList getRegister(Scanner scanner) {
        IntList register = new IntArrayList();
        register.add(1);
        register.add(1);
