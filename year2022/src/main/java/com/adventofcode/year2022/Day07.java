...
public final class Day07 {
    ...
    static final class PartOne {
        ...
        static long findSum(Scanner scanner) {
            Directory root = readInput(scanner);

            Map<Directory, Long> sizes = findSize(root);
            return sizes.values().stream().filter(s -> s <= 100000).mapToLong(i -> i).sum();
        }
    }
    ...
    static final class PartTwo {
        ...
        static OptionalLong findSmallestToDelete(Scanner scanner) {
            Directory root = readInput(scanner);
            ...
            long toDelete = neededSpace - unusedSpace;
            return sizes.values().stream().filter(s -> s > toDelete).mapToLong(i -> i).min();
        }

    }


    static Map<Directory, Long> findSize(Directory root) {
        Map<Directory, Long> sizes = new HashMap<>();
        Queue<Directory> queue = new ArrayDeque<>();
        queue.add(root);
        while (!queue.isEmpty()) {
            Directory current = queue.poll();
            sizes.put(current, current.size());
            queue.addAll(current.subdirectory);
        }
    ...
    static Directory readInput(Scanner scanner) {
    ...
    static void printDirectory(Directory d, int depth) {
        String repeated = "  ".repeat(depth);
        LOGGER.info("{}- {} (dir)", repeated, d.name);
        for (Directory directory : d.subdirectory) {
            printDirectory(directory, depth + 1);
        }
        for (File file : d.files) {
            LOGGER.info("{}  - {} (file, size={})", repeated, file.filename, file.size);
        }
    }


    record File(long size, String filename) {
    ...
    record Directory(String name, Directory parent, List<File> files, List<Directory> subdirectory) {
        ...
        public long size() {
            return files.stream().mapToLong(File::size).sum()
                   + subdirectory.stream().mapToLong(Directory::size).sum();
        }
